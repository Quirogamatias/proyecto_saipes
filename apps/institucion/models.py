...
class Horario(models.Model):
    ...
    activo = models.BooleanField('Horario Usado/no Usado', default= True)
...
class Alumno(models.Model):
    ...
    def obtener_carreras(self):
        carreras = str([carrera for carrera in self.id_carrera.all().values_list('carrera',flat = True)]).replace("[","").replace("]","").replace("'","")
        return carreras
...
class Fecha(models.Model):
    ...
    tipo_evento = models.CharField('Tipo',max_length=100,choices=tipo, null=False, blank = False)  
...
class Pregunta(models.Model):
    ...
    def natural_key(self):
        return f'Pregunta {self.asunto}, descripcion {self.descripcion}'
...
class PromedioNotasTotal(models.Model):
    id_pormedionotastotal= models.AutoField(primary_key= True)  
    id_alumno = models.ForeignKey(Alumno, on_delete=models.CASCADE) 
    cantidad = models.IntegerField('Cantidad de notas',null=False, blank = False,default=0)
    suma = models.FloatField('Suma de notas',null=False, blank = False,default=0)
    total = models.FloatField('Total de notas',null=False, blank = False,default=0)
    ...
    def natural_key(self):
        return f' Alumno {self.apellido}, Promedio {self.total}'
   
    class Meta:
        verbose_name = 'Promedio de Notas Total'
        verbose_name_plural = 'Promedio de Notas Total'
        ordering = ['id_pormedionotastotal']

    def __str__(self):
        return f'Alumno {self.id_alumno}, Promedio {self.total}'