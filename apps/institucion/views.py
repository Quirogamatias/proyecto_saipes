...
class InicioPromedioNotasTotal(LoginMixin, TemplateView):
    template_name = 'institucion/promedio/listar_promediot.html'
    permission_required = ('institucion.view_promedionotasfinal', 'institucion.add_promedionotasfinal',
                           'institucion.delete_promedionotasfinal', 'institucion.change_promedionotasfinal')
...
class InicioInscripcionAlumno(ValidarAlumno, TemplateView):
    template_name = 'institucion/inscripcion/listar_inscripcion_alumno.html'
    permission_required = ('institucion.view_inscripcion', 'institucion.add_inscripcion',
                           'institucion.delete_inscripcion', 'institucion.change_inscripcion')
...
class CrearAlumno(ValidarAdministrador,CreateView):
    ...
    fifth_model= PromedioNotasTotal
    fifth_form_class = PromedioNotasTotalForm
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'form3' not in context:
            context['form3'] = self.fifth_form_class(self.request.GET)
        return context
...
class EliminarAlumno(ValidarAdministrador,DeleteView):
    ...
    def delete(self,request,pk,*args,**kwargs):
        if request.is_ajax():
            ...
            promedionotastotal = self.fifth_model.objects.all()
            for k in range(len(promedionotastotal)):
                if promedionotastotal[k].estado == True:
                    if promedionotastotal[k].id_alumno == alumno:
                        promedionotastotal[k].estado=False
                        promedionotastotal[k].save()
            alumno.estado = False
            alumno.save()
            mensaje = f'{self.model.__name__} eliminacion correctamente!'
            error = 'No hay error!'
...
class ListadoInscripcionesAlumno(ValidarAlumno,View):
    model = Inscripcion
    second_model = Alumno
    permission_required = ('institucion.view_inscripcion', 'institucion.add_inscripcion',
                           'institucion.delete_inscripcion', 'institucion.change_inscripcion')  
            ...
            if alumno[i].estado == True:
                for j in range(len(inscripcion)):
                    if inscripcion[j].estado == True:
                        if alumno[i]==inscripcion[j].id_alumno:
    ...
    def get(self,request,*args,**kwargs):
        if request.is_ajax():            
            ...
            return redirect('institucion:inicio_inscripcion_alumno')
...
class CrearInscripcionAlumno(ValidarAlumno,CreateView):
    model = Inscripcion
    second_model = PromedioAsistencia
    third_model = PromedioNotasFinal
    fourth_model = PromedioNotasParcial
    ...
    seventh_model=Carrera
    sixth_model = Materia
    template_name = 'institucion/inscripcion/crear_inscripcion_alumno.html'
    permission_required = ('institucion.view_inscripcion', 'institucion.add_inscripcion',
                           'institucion.delete_inscripcion', 'institucion.change_inscripcion')
    #nose porque no me guarda cuando uso el inscripcionform en vez del inscripcio2form
    @method_decorator(csrf_exempt)
    ...
    def get_context_data(self,**kwargs):
        ...
        #carrera=self.seventh_model.objects.all()
        for i in range(len(a)):
            if a[i].estado == True:                
                ins=self.model.objects.filter(id_alumno=a[i])
                print (ins)
        ...
        context["carrera"] = self.seventh_model.objects.all()
        return context
    ...
    def post(self,request,*args,**kwargs):
        ...
        inscripcion = self.model.objects.filter(id_alumno =nuevo.id_alumno, id_materia = nuevo.id_materia )

        nuevo2= PromedioAsistencia(
            id_alumno=nuevo.id_alumno,
            id_materia=nuevo.id_materia
        )
        nuevo3= PromedioNotasParcial(
            id_alumno=nuevo.id_alumno,
            id_materia=nuevo.id_materia
        ...
        if not inscripcion:
            ...
            nuevo3.save()
            nuevo4.save()
            return render(request,'institucion/inscripcion/mensaje_inscripto.html') 
        else:
            for i in range(len(inscripcion)):
                if inscripcion[i].estado == True:
                    if inscripcion[i].id_alumno == nuevo.id_alumno and inscripcion[i].id_materia == nuevo.id_materia:
                        b=1
                        return render(request,'institucion/inscripcion/mensaje_existe.html') 
        ...
        return render(request,'institucion/inscripcionexamen/listar_inscripcion_alumno.html') 
...
class EliminarInscripcionAlumno(ValidarAlumno,DeleteView):
    model = Inscripcion
    second_model = PromedioAsistencia
    third_model = PromedioNotasFinal
    fourth_model = PromedioNotasParcial
    fifth_model = Asistencia
    sixth_model = Notas
    template_name = 'institucion/inscripcion/eliminar_inscripcion_alumno.html'
    permission_required  = ( 'institucion.view_inscripcion' , 'institucion.add_inscripcion' ,
                           'institucion.delete_inscripcion' , 'institucion.change_inscripcion' )
...
class ActualizarMateria(ValidarAdministrador,UpdateView):
    ...
    def get_context_data(self, **kwargs):        
        context = super(ActualizarMateria, self).get_context_data(**kwargs)
        if 'form' not in context:
            context['form'] = self.form_class(self.request.GET)
        return context
                            ...
                            if materia[i].id_materia==nuevo.id_materia: 
                                ...
                                #print(materia[i].id_horario.all())
                                            if horario[j] in materia[i].id_horario.all():
                                                ...
                                                return response 
                if a==1:
                    horario2 = self.second_model.objects.all()
                    for i in range(len(materia)):
                        if materia[i].estado == True:
                            if materia[i].id_materia == nuevo.id_materia:
                                if materia[i].materia==nuevo.materia:                                  
                                    for j in range(len(horario2)):
                                        if horario2[j].estado== True:  
                                            if horario2[j] in materia[i].id_horario.all():   
                                                horario2[j].activo= True
                                                horario2[j].save()
                    ...
                    for j in range(len(horario)):
                        if horario[j].estado== True:
                            for i in range(len(materia)):
                                if materia[i].estado == True:            
                                    if horario[j] in materia[i].id_horario.all(): 
                                        horario[j].activo= False
                                        horario[j].save()
                                    else:
                                        if horario[j].activo == True:
                                            horario[j].activo= False
                                            horario[j].save() 
...
class EliminarMateria(ValidarAdministrador,DeleteView):
    ...
    def delete(self,request,pk,*args,**kwargs):
        if request.is_ajax():            
            ...
            #horario = self.second_model.objects.filter(id_horario__in=materia.id_horario)
            horario = self.second_model.objects.all()
            for i in range(len(horario)):
                if horario[i] in materia.id_horario.all():
                    horario[i].activo=True
                    horario[i].save()                  
...
class ActualizarNotas(ValidarProfesor,UpdateView):
    ...
    fourth_model = Inscripcion
    fifth_model= Alumno
    eighth_model = PromedioNotasTotal
    ...
    def post(self,request,*args,**kwargs):
        if request.is_ajax():
            ...
            if form.is_valid():
                ...
                promedioTotal = self.eighth_model.objects.all()
                                ...
                                if nuevo.id_materia == promedioF[i].id_materia and nuevo.id_alumno == promedioF[i].id_alumno:
                                    ...
                                    for j in range(len(notas)):
                                        ...
                                        #promedioF[i].cantidad = promedioF[i].cantidad + 1
                                                ...
                                                for k in range(len(promedioTotal)):
                                                    if promedioTotal[k].estado==True:
                                                        if promedioTotal[k].id_alumno == promedioF[i].id_alumno:
                                                            if promedioTotal[k].cantidad == 0:
                                                                promedioTotal[k].cantidad=1 
                                                            promedioTotal[k].suma = promedioTotal[k].suma + nuevo.notas - notas[j].notas
                                                            promedioTotal[k].total = promedioTotal[k].suma / promedioTotal[k].cantidad
                                                            promedioTotal[k].save()
                                                form.save() 
                                                promedioF[i].save()
                                                mensaje = f'{self.model.__name__} registrado correctamente!'
                                                error = 'No hay error!'
                                                                ...
                                                                if nuevo.id_materia == promedioF[i].id_materia and nuevo.id_alumno == promedioF[i].id_alumno:
                                                                    if promedioF[i].cantidad < 4:
                                                                        ...
                                                                        for k in range(len(promedioTotal)):
                                                                            if promedioTotal[k].estado==True:
                                                                                if promedioTotal[k].id_alumno == promedioF[i].id_alumno:
                                                                                    promedioTotal[k].cantidad = promedioTotal[k].cantidad + 1
                                                                                    promedioTotal[k].suma = promedioTotal[k].suma + nuevo.notas
                                                                                    promedioTotal[k].total = promedioTotal[k].suma / promedioTotal[k].cantidad
                                                                                    promedioTotal[k].save()
                                                                        form.save() 
                                                                        promedioF[i].save()
...
class CrearNotasP(ValidarProfesor,CreateView):
    ...
    tenth_model = Carrera
    eleventh_model = PromedioNotasTotal
    ...
    def post(self,request,*args,**kwargs):  
        ...
        if request.is_ajax():
            ...
            if form.is_valid():
                ...
                promedioF = self.second_model.objects.all()
                promedioP = self.third_model.objects.all()
                promedioTotal = self.eleventh_model.objects.all()
                inscripcion = self.fourth_model.objects.all()
                alumno = self.fifth_model.objects.all() 
                                ...
                                if notas2[j].id_notas == notas.id_notas:
                                    ...
                                    for k in range(len(promedioTotal)):
                                        if promedioTotal[k].estado==True:
                                            if promedioTotal[k].id_alumno == promedioF[i].id_alumno:
                                                promedioTotal[k].cantidad = promedioTotal[k].cantidad - 1
                                                promedioTotal[k].suma = promedioTotal[k].suma - notas.notas
                                                promedioTotal[k].total = promedioTotal[k].suma / promedioTotal[k].cantidad
                                                promedioTotal[k].save()                                  
                                    promedioF[i].save()
...
class DetalleCarrera(ValidarAdministrador,DetailView):
    model = Carrera
...
class DetalleMateria(ValidarAdministrador,ListView):
    model = Materia    
    ...
    #def get_queryset(self):
     #   return self.model.objects.filter(estado = True)  
class DetalleMateriaAlumno(ValidarProfesorA,ListView):
    model = Materia    
    second_model = Inscripcion
    third_model = Alumno
    ...
    def get_context_data(self,**kwargs):
        ...
        context["alumnos"] = self.third_model.objects.all()
        return context
class ListadoMateriaAlumno(ValidarProfesorA,ListView):
    model = Materia
...
class DetalleInscripcion(ValidarAdministrador,ListView):#si es listview me muestra todos los datos en un objeto,si es un detailview me muetraun solo dato del pk
    model = Inscripcion
...
class DetalleInscripcionAlumno(ValidarAlumno,ListView):
    model = Inscripcion
    second_model = Materia
    permission_required = ('institucion.view_inscripcion', 'institucion.add_inscripcion',
                           'institucion.delete_inscripcion', 'institucion.change_inscripcion')  
    template_name = 'institucion/inscripcion/detalle_inscripcion_alumno.html'
    ...
    def get_context_data(self):
        pk = self.kwargs.get('pk')
        inscripcion = self.model.objects.get(pk= pk)
        materia= self.second_model.objects.get(materia= inscripcion.id_materia)
        return {'inscripcion': inscripcion,'materia': materia}
...
class AsistenciaMateria(ValidarAdministrador,ListView):
    model = Materia    
...
class EnviarMensaje(ValidarAdministrador,ListView):
...
class MensajeAdvertencia(ValidarAdministrador,ListView):
...
class MensajeAdvertenciaAsistencia(ValidarAdministrador,ListView):
                        ...
                        if promedioasistencia[k].id_alumno == alumno[i]:                                                   
                            for j in range(len(materia)):
                                if materia[j].estado == True and promedioasistencia[k].id_materia == materia[j]:
                                    ...
                                    #email_desde = settings.EMAIL_HOST_USER
                                        mensajes="Se comunica que el alumno: "+alumno[i].apellido +",que tiene una asistencia menor al 75% de la Materia: " + materia[j].materia
                                        email_para= "sistema.academico.ipes@gmail.com",alumno[i].email
                                        send_mail(asunto,mensajes,email_desde,email_para, fail_silently=False)
...
class MensajeEvento(ValidarAdministrador,ListView):#no lo hice todavia
    ...
    def post(self,request,*args,**kwargs):
        ...
        if request.is_ajax():
            ...
            if form.is_valid():
                ...
                fecha = self.fourth_model.objects.filter(tipo_evento="Examen")
                print (fecha)
                for j in range(len(inscripcion)):
                    if inscripcion[j].estado == True:
...
class TestView(ValidarProfesor,TemplateView):
    model = Materia
    second_model = Carrera
...
class ListadoRespuestas(ValidarAdministrador,ListView):
    model = Respuesta   
    permission_required = ('institucion.view_respuesta', 'institucion.add_respuesta',
                           'institucion.delete_respuesta', 'institucion.change_respuesta')
...
class ListadoPromedioTotal(LoginMixin,ListView):
    model = PromedioNotasTotal
    permission_required = ('institucion.view_promedionotastotal', 'institucion.add_promedionotastotal',
                           'institucion.delete_promedionotastotal', 'institucion.change_promedionotastotal')
    ...
    def get_context_data(self,**kwargs):
        contexto= {}
        contexto ['promedionotastotal'] = self.get_queryset() #agregamos la consulta al contexto
        contexto['form'] = self.form_class
        return contexto
    ...
    def get(self,request,*args,**kwargs):
        if request.is_ajax():            
            ...
            return redirect('institucion:InicioPromedioNotasTotal')

#tengo que optimizar cirtos template, para que funcionen mejor
...
#agregar template para el alumno listado inscripcion-listo
#hacer que las pregunta solo puedan ser vista por el admin
#Actualizar fecha-listo
#mejorar template de mensajes-listo
#modifique el models de horario, cree un form2 y actulice el crear y eliminar de materia, y actualizar=listo
#inscripcion examen tngo que modificar el editar para que me muestre solo el alumno que quiero,listo
#agregue e import_export al admin-listo
#cuando inscribo una nota menor a 6, automaticamente se enviar un mensje al alumno-listo
#me falta cambiar los validar a algunos vistas-listo