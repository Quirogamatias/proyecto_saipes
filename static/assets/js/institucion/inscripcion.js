...
function listadoInscripcionesAlumno(){
    $.ajax({
        url: "/institucion/listar_inscripciones_alumno/",
        type: "get",
        dataType: "json",
        success: function(response){
            if($.fn.DataTable.isDataTable('#tabla_inscripciones_alumno')){
                $('#tabla_inscripciones_alumno').DataTable().destroy();
            }
            $('#tabla_inscripciones_alumno tbody').html("");
            for(let i = 0;i < response.length;i++){
                let fila = '<tr>';
                fila += '<td>' + (i+1) + '</td>';
                fila += '<td>' + response[i]["fields"]['fecha_inscripcion'] + '</td>';
                ...
                fila += '<td> <button type = "button" class = "btn btn-primary btn-sm tableButton"';
                fila += 'onclick = "abrir_modal_eliminacion(\'/institucion/eliminar_inscripcion_alumno/'+ response[i]['pk']+'/\');"> ELIMINAR </button>';
                fila += '<button type = "button" class = "btn btn-info tableButton btn-sm"';
                fila += 'onclick="location.href=(\'/institucion/detalle_inscripcion_alumno/'+ response[i]['pk']+'/\');"> DETALLE </button> </td>';
                fila += '</tr>';
                $('#tabla_inscripciones_alumno tbody').append(fila);
            }             
            $('#tabla_inscripciones_alumno').DataTable({
                language: {
                    "decimal": "",
                    "emptyTable": "No hay información",
                    "info": "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
...
function eliminarA(pk){
	$.ajax({
		...
		url: '/institucion/eliminar_inscripcion_alumno/'+pk+'/',
		type: 'post',
		success: function(response) {
            notificacionSuccess(response.mensaje);
			listadoInscripcionesAlumno();
			cerrar_modal_eliminacion();
		},
		error: function(error) {
            notificacionError(error.responseJSON.mensaje);
